...
function DoctorPatients() {
  ...
  const [receptionistInfo] = useState(JSON.parse(localStorage.getItem('Receptionist')) ?? []);
  const patientsFromDoctor = async () => {
    setIsLoading(true);
    const staffId = receptionistInfo.user.uuid;
    console.log(staffId);
    if (user) {
              ...
              {isLoading ? (
                ...
                patientsList.map((data, key) => {
                  ...
                  return (
                    <li key={key}>
                      <Link
                        to={`/patient-invoice/${session.id}/${Patient.uuid}`}
